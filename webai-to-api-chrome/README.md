# WebAI-to-API (System Google Chrome Edition)

> 🚀 **Forked version configured to use Google Chrome installed on your machine** instead of downloading Playwright's bundled Chromium.
>
> *(Phiên bản tùy biến tự động sử dụng Google Chrome có sẵn trên máy tính của bạn, không cần tải thêm Chromium từ Playwright).*
...
* **Uses Host Google Chrome**: Automatically detects and uses Google Chrome already installed on your machine (Windows, macOS, or Linux).
* **No Huge Downloads**: Bypasses the ~280MB `playwright install chromium` requirement.
* **Better Anti-bot & Stealth**: Uses your real Chrome browser binary, reducing automation detection on Google Gemini.
* **Auto-Detection & Configurable**: Automatically locates Chrome in standard directories or accepts custom `chrome_path` in `config.conf`.

### ⚙️ Chrome Configuration (`config.conf`)

```ini
[Browser]
name = chrome
runtime = playwright
# Use Google Chrome installed on your computer
use_system_chrome = true
# Channel to launch (chrome, chrome-beta, msedge)
chrome_channel = chrome
# Optional: specify custom path if Chrome is installed in non-standard folder
